...
window.SCENARIO_DATA = {
  ...
  statusTexts: {
    trust: [
      { max: 12, text: '天使様との距離：まだ少し遠い' },
      { max: 28, text: '天使様との距離：少しずつ言葉が増えている' },
      { max: 60, text: '天使様との距離：穏やかな信頼が芽生えている' },
    ],
    prayerTuning: [
      { max: 12, text: '祈りの響き：まだ不安定' },
      { max: 28, text: '祈りの響き：かすかに整っている' },
      { max: 60, text: '祈りの響き：この場所に静かに届いている' },
    ],
    caretakerAptitude: [
      { max: 12, text: '部屋の空気：まだ手探り' },
      { max: 28, text: '部屋の空気：少しずつ整ってきた' },
      { max: 60, text: '部屋の空気：あなたの手が馴染んでいる' },
    ],
    mentalMargin: [
